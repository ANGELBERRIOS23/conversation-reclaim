...
1. **By an AI agent (recommended)** — the repo ships an agent skill
   ...
   author uses it.
2. **Visual interface** — the portable app uses Qt 6 with a modern,
   responsive macOS/Windows interface and an Español/English selector that
   remembers your choice. Run `python3 reclaim.py gui` on macOS/Linux or
   ...
   From a source checkout on macOS, double-click **`Conversation Reclaim.app`**;
   on Windows, use `launch-gui.bat`. These source launchers use an installed
   Python/Tk. The portable downloads below bundle their own runtime, while
   `launch-gui.command` remains a terminal fallback.
...
can provide three self-contained downloads generated by GitHub Actions:

- **Windows x64 portable:** unzip and double-click `Conversation Reclaim.exe`.
...
Those packages include their own Python and Qt 6 runtime; end users install
nothing and do not need Python. The source
launchers remain available for contributors who already have Python. Unsigned
development builds may require **right-click → Open** on macOS or confirmation
through Windows SmartScreen. A tagged version (`v*`) builds all three packages
and attaches them to its GitHub release; the workflow can also be run manually.
...
To build the native package for the current operating system:
